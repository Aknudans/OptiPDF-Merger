...
- The whole pipeline call (`merge_pdfs`/`deduplicate_if_available`/`compress_pdf`) runs inside `contextlib.redirect_stdout(...)` **and** `contextlib.redirect_stderr(...)` simultaneously, both wrapped in `_TeeWriter`. Both streams are required: `tqdm.write()` (the phase messages) goes to **stdout** by default, but the `tqdm(...)` progress bar itself goes to **stderr** by default — verified empirically, not assumed. Missing either one loses half the picture.
- `_TeeWriter` writes every chunk to *two* places at once: the `io.StringIO` capture buffer (for the browser's log panel) and the real original `sys.stdout`/`sys.stderr` (captured *before* redirection, so it's the actual console). This makes the same phase-by-phase progress print live in the cmd window that `run_gui.bat` opened — the concrete problem it solves is a multi-file compression run taking a while with a silent GUI and no way to tell if it's frozen; now the terminal shows the same thing it would during a CLI run.
